...
I will indicate with (H1) the heading 1 style in Google Docs, (H2) the heading 2; use the same labels and styles I'm using unless specified otherwise.
...
Normal dream can't get less than 0.5 and will get a score in increments of 0.5, you cna see the table below for reference.
Very long dream can get a score bigger than 2, there is no max.

The **total** `Score` is simply the sum of `D` and `LD`. You can update it with **✏️ Fill In Scores**, or anyway they will updated once the function to create the **Score Table** is called.
...
The dates in the table are also links that will jump to the matching date in the doc. <br>
...
The **Average and median** table can be generated by using **⬜ Update Statistic Dream Lines**, but i suggests to make your own after you have several entries.  
If you want to check them anyway, you can run the function and an alert will show them, then you can answer **No** to the successive prompt that ask to update them.
...
Once you have your own you can simply use **🗓️🗓️🗓️ Update Dream Score on All Days** to update all entries based on your statics.<br>
...
The histogram is upgraded when the statics are calculated using **'⬜ Update Statistic Dream Lines'**, <br>
but you can update or generate it without triggering a statistics update by using **'📊 Update Histogram Dream Lines'** <br>
The full histogram will show value from 1 line dream until 49 lines, 50 or more lines dreams will end in the bucket 50+. <br>
